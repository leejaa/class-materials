# -*- coding: utf-8 -*-
"""TSC 기출 2회(유튜브 영상) → 문제·모범답안(3/4/5급) docx. 기존 실전문제집 서식 준용."""
...
def build(data, ans, out, LV=None):
    ...
    _t = f'TSC 기출 2회 — 문제 · 모범답안 ({LV})' if LV else 'TSC 기출 2회 — 문제 · 모범답안 (3급 / 4급 / 5급)'
    p = para(after=1); run(p, _t, size=16, bold=True)
    p = para(after=2); run(p, '출처: TSC 기출문제집 온라인 영상 테스트 (기출 문제 02회)', size=9, color=GRAY)
...
if __name__ == "__main__":
    data = json.load(open(os.path.join(HERE, "tsc_yt02.json"), encoding="utf-8"))
